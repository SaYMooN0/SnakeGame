...
namespace SnakeGameWpf
{
    internal class AllTheLogic
    {
        ...
        public Canvas showSnake()
        {
            Canvas snakeToShow = new Canvas();
            for (int i = 0; i < 13; i++)
            {
                for (int j = 0; j < 13; j++)
                {
                    Canvas part = field[i, j].show(snake);
                    snakeToShow.Children.Add(part);
                    Canvas.SetTop(part, winSize / 13 * i);
                    Canvas.SetLeft(part, winSize / 13 * j);
                }
            }
            return new Canvas();
        }
